 ...
 *
 * Generate customized HTML for several EmailId values, if EmailId is not part of the value, delegate to the normal EmailTemplate
 *
 * @package OCA\CustomizedEmails
 * 
 * Would be better designed with: implements IEMailTemplate
 * Mailer.php must be changed
 */
class CustomizedEmails extends EMailTemplate {
...
*/
	...
	public function addFooter(string $text = '', ?string $lang = NULL) {
		if(isset($this->delegate)) {
			return( $this->delegate->addFooter($text,$lang));
		}
	}
